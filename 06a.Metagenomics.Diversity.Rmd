...
#pdf("/ebio/abt3_projects2/uHEAT/data/RevisionPlots/3D.BrayTemporalFever.pdf", width=4, height=3)
p <- ggplot(brayq2_meta, aes(x=Time01, y=Bray_Distance, colour=fever7d_aboveavg)) + 
  ...
  annotate('text', x=3, y=0.65, label=c("Time p=0.41\nFever p=0.47\nTime:Fever p=0.85"), hjust=0, size=2.5)
p
#dev.off()
...
#pdf("/ebio/abt3_projects2/uHEAT/data/RevisionPlots/3E.ShannonTemporalFever.pdf", width=4, height=3)
p <- ggplot(pdata, aes(x=Time01, y=shannon_entropy, colour=fever7d_aboveavg)) + 
  ...
  scale_x_continuous(breaks = (seq(-7, 7, by = 7))) + 
  scale_colour_manual(values=c("grey50", "firebrick3")) + 
  annotate('segment', x=0, xend=0, y=5, yend=10, linetype='dashed') +
  annotate('text', x=3, y=9.5, label=c("Time p=0.50\nFever p=0.83\nTime:Fever p=0.28"), hjust=0, size=2.5)
p
#dev.off()
