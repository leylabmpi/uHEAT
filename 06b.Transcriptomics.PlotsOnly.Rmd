...
meta_export <- meta_seq %>% select(Participant_ID, SampleID, RNA_seq_depth)
...
p <- ggplot(pdata, aes(x=UniRef_annotation, y=log(RelAbund), colour=fever7d_aboveavg)) + geom_boxplot(outlier.shape=NA) +
  ...
  ylab("Log RNA Rel. Abund.") + labs(colour="∆T") + xlab(NULL) + 
...
export2 <- genes_rnadna3 %>% filter(qval<0.1) %>% dplyr::select(feature, annotation, value, coef, pval, qval, N, N.not.zero)
export2$Covariates <- c("seq_depth+BSS+Time_at_RT+Age+Sex+Diet")

#add it to the other table
export_rnadna <- rbind(vdata_export, export2)

write.csv(vdata_export, "/ebio/abt3_projects2/uHEAT/data/SupplementalTables/Table#_Fig4A.csv")
...
p2 <- ggplot(pdata2, aes(x=Uniref, y=log(RNA_vs_DNA), colour=fever7d_aboveavg)) + geom_boxplot(outlier.shape=NA)  +
  geom_point(position=position_jitterdodge(jitter.width=0.1, jitter.height=0)) + 
  scale_colour_manual(values=c("grey50", "firebrick3")) + theme_bw() + 
  ylab("Log RNA Rel. Expr.") + labs(colour="∆T") + xlab(NULL) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
p2
#dev.off()
...
p5 <- ggplot(plot_genes, aes(x=as.factor(sample_no3), y=log(UniRef50_R6ZZ21..Flagellin.FliC1), colour=fever7d_aboveavg)) +
  ...
  xlab("Timepoint") + ylab("Log RNA Rel. Expr.") + ggtitle("Flagellin") + labs(colour="∆T")
p5
#dev.off()
...
p <- ggplot(csra, aes(x=Diet3, y=log(UniRef50_A0A1C2Y4D4), colour=fever7d_aboveavg)) + geom_boxplot(outlier.size=NA, outlier.shape=NA) + 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
               ...
               axis.title.y = element_text(margin = margin(r = 10))) + 
  scale_colour_manual(values=c("grey50", "firebrick3")) + 
  xlab("Diet") + ylab("Log RNA Rel. Expr.") + ggtitle("CsrA Translational Regulator") + labs(colour="∆T") +
...
#edit the genus category to include family as well
pdata$Genus <- paste("f_", pdata$Family, ";g_", pdata$Genus, sep="")
...
#pdf(width=4, height=3, "/ebio/abt3_projects2/uHEAT/data/CleanPlots/4D_Flagellin_Genus_Summary.pdf")
...
p = ggplot(data=vdata, aes(x=coef, y=-log10(pval), 
                              col=diffexpressed, 
                              label=delabel)) + 
  ggtitle(expression("RNA ∆T"^hi~"After Vaccine")) +
  scale_colour_manual(values = mycolours) + 
  geom_point(size=0.3) + 
  theme_bw() +
  guides(fill = guide_legend(override.aes = aes(label = ""))) + 
